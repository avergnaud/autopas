...
def load_config() -> dict[str, Any]:
    ...
    Returns:
        ...
        oauth2, session, server, questions, authorized_users.
    """
    global _config

    # Load .env so environment variables are available before reading app.yaml.
    # Try BASE_DIR/.env (production); fall back to cwd search (dev without PAS_BASE_DIR).
    env_file = BASE_DIR / ".env"
    if env_file.exists():
        load_dotenv(env_file)
        logger.info("Loaded .env from %s", env_file)
    else:
        found = load_dotenv()  # searches cwd and parent dirs
        if found:
            logger.info("Loaded .env from cwd (fallback)")
        else:
            logger.warning(".env not found — secrets must be set in environment")
