...
# ---------------------------------------------------------------------------
# Politiques sécurité — upload / anonymize / list / delete
# ---------------------------------------------------------------------------
...
async def upload_policy(
    ...
    """Upload a security policy .docx file.
    ...
    Returns:
        policy_id and original filename.
    """
    ...
    original_path = policy_dir / "original.docx"
    content = await file.read()
    original_path.write_bytes(content)

    meta: dict = {"filename": file.filename, "format": "docx"}
    (policy_dir / "metadata.json").write_text(
        json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8"
    )

    logger.info(
        "Policy %s created by %s — %s (%d bytes)",
        policy_id, user["email"], file.filename, len(content),
    )
    return {"policy_id": policy_id, "filename": file.filename}


@router.get("/policies/{policy_id}/anon-suggestions")
async def policy_anon_suggestions(
    policy_id: str,
    user: dict = Depends(get_current_user),
) -> dict:
    """Extract metadata suggestions from a policy docx for the anonymization step.

    Args:
        policy_id: UUID of the policy entry.
        user: The authenticated user (injected by dependency).
    ...
    _generic = {"Microsoft Word", "Microsoft Office Word"}
    seen: set[str] = set()
    suggestions: list[str] = []
    for v in file_meta.values():
        if v and v not in seen and v not in _generic:
...
@router.post("/policies/{policy_id}/anonymize")
async def policy_anonymize(
    policy_id: str,
    body: AnonymizeRequest,
    user: dict = Depends(get_current_user),
) -> dict:
    """Anonymize a policy docx: replace keywords and strip file metadata.
    ...
    original_path = policy_dir / "original.docx"
    anonymized_path = policy_dir / "anonymized.docx"
    ...
    mapping = await anyio.to_thread.run_sync(
        lambda: anonymize_docx(original_path, anonymized_path, raw_mapping, strip_layout=True)
    )

    map_path = policy_dir / "anonymized_map.json"
    map_path.write_text(json.dumps(mapping, ensure_ascii=False, indent=2), encoding="utf-8")

    logger.info(
        "Policy %s anonymized by %s — %d keywords", policy_id, user["email"], len(mapping)
    )
    return {"status": "ok", "mapping": mapping}
...
async def list_policies(user: dict = Depends(get_current_user)) -> dict:
    ...
    for d in sorted(POLICIES_DIR.iterdir()):
        ...
        if meta_path.exists():
            meta = json.loads(meta_path.read_text(encoding="utf-8"))
            entries.append({"policy_id": d.name, **meta})
...
async def delete_policy(
    ...
    """Delete a policy entry (file + metadata).
    ...
    logger.info("Policy %s deleted by %s", policy_id, user["email"])
    return {"status": "deleted"}
...
async def download_policy(
    ...
    """Download the anonymized policy docx (falls back to original if not yet anonymized).
    ...
    anonymized = policy_dir / "anonymized.docx"
    original = policy_dir / "original.docx"
    path = anonymized if anonymized.exists() else original
    if not path.exists():
        raise HTTPException(status_code=404, detail="Fichier introuvable.")

    stem = Path(filename).stem
    download_name = f"{stem}_anonymized.docx" if path == anonymized else filename
    return FileResponse(
        path=str(path),
    ...
    result = [
        {
            ...
            "secteur_client": e.get("secteur_client", ""),
            "nb_etp": e.get("nb_etp"),
