...
# ---------------------------------------------------------------------------
# Download prompt debug — GET /api/projects/{project_id}/prompt
# ---------------------------------------------------------------------------
...
async def download_prompt(
    ...
    """Download the raw prompts sent to Claude during generation.
    ...
    prompt_path = PROJECTS_DIR / project_id / "prompt_debug.txt"
    if not prompt_path.exists():
        raise HTTPException(status_code=404, detail="Prompt non disponible.")

    return FileResponse(
        path=str(prompt_path),
        media_type="text/plain; charset=utf-8",
        filename=f"prompt_debug_{project_id[:8]}.txt",
    )
