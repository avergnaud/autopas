...
def _parse_json(text: str, stop_reason: str = "") -> dict:
    """Extract and parse JSON from Claude response, stripping markdown fences.

    Logs the raw response on failure so truncation issues are diagnosable.
    """
    if stop_reason == "max_tokens":
        logger.warning(
            "_parse_json: stop_reason=max_tokens — response was truncated, JSON may be incomplete"
        )
    cleaned = text.strip()
    ...
    try:
        ...
        logger.error(
            "_parse_json failed (%s) — stop_reason=%r — raw response (last 500 chars): %r",
            exc,
            stop_reason,
            text[-500:],
        )
...
def analyze_structure(raw_content: str, fmt: str) -> dict:
    ...
    return _parse_json(response_text, stop_reason=message.stop_reason)
...
def generate_responses(
    ...
    data = _parse_json(message.content[0].text, stop_reason=message.stop_reason)
