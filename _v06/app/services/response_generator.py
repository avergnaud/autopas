...
def _snapshot(src: Path, project_id: str, label: str) -> None:
    """Save a copy of src into data/projects/{id}/debug/{label} for post-mortem analysis."""
    from app.services.project_manager import PROJECTS_DIR
    debug_dir = PROJECTS_DIR / project_id / _DEBUG_DIR_NAME
    debug_dir.mkdir(parents=True, exist_ok=True)
    dest = debug_dir / label
    shutil.copy2(src, dest)
    logger.info("[SNAP %s] %s → %s", project_id, src.name, dest.name)


def _format_cadrage(cadrage: dict) -> str:
    """Format cadrage answers using question text labels instead of raw numeric keys."""
    config = get_config()
    raw_questions: list[dict] = config.get("questions", [])
    # Build a lookup: question id (1-based str) -> question text
    label_map = {str(idx + 1): q.get("text", "") for idx, q in enumerate(raw_questions)}
...
def run_generation(project: Project) -> None:
    ...
    try:
        ...
        if project.format == "xlsx":
            _snapshot(working_path, project.id, "01_after_copy.xlsx")
        ...
        if project.format == "xlsx" and project.structure and project.structure.sheets:
            ...
            _snapshot(working_path, project.id, "02_after_delete_sheets.xlsx")
        ...
        if anonymizer:
            ...
            if project.format == "xlsx":
                ...
                _snapshot(working_path, project.id, "03_after_anonymization.xlsx")
            else:
        ...
        if project.format == "xlsx":
            ...
            _snapshot(output_path, project.id, "04_after_write_responses.xlsx")
        else:
        ...
        if anonymizer:
            ...
            if project.format == "xlsx":
                ...
                _snapshot(output_path, project.id, "05_after_deanonymization.xlsx")
            else:
