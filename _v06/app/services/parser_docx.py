...
def extract_questions(filepath: Path, structure: DocumentStructure) -> list[dict]:
    ...
    # When a marker cell is found, look for the question text in the other cells
    # of the same row, then in the previous row as fallback.
    for table in doc.tables:
        ...
        for row_idx, row in enumerate(rows):
            ...
            # Collect text from non-marker cells in this row
            q_text = ""
            for cell in cells:
                cell_text = cell.text.strip()
                if cell_text and response_marker not in cell_text.lower():
                    q_text = cell_text
                    break

            # Fallback: use previous row's first non-empty cell
            if not q_text and row_idx > 0:
