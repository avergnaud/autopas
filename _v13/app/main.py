...
# Try BASE_DIR/.env (production) first; fall back to searching from cwd (dev
# without PAS_BASE_DIR set, e.g. `uvicorn app.main:app` run from _vXX/).
_env_file = BASE_DIR / ".env"
if _env_file.exists():
    load_dotenv(_env_file)
else:
    load_dotenv()  # searches cwd and parent dirs — finds _vXX/.env in dev
...
async def lifespan(app: FastAPI):
    ...
    load_config()
    project_manager.recover_stale_projects()
